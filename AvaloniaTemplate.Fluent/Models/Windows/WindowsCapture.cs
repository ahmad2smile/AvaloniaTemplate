...
using static AvaloniaTemplate.Fluent.Models.Windows.DirectShow;
...
public class WindowsCapture
{
	...
	{
		var sz = 0;
		i_grabber.GetCurrentBuffer(ref sz, nint.Zero);
		if (sz == 0)
		{
			return null;
		}
	...
	private static T? PtrToStructure<T>(nint ptr) => (T?)Marshal.PtrToStructure(ptr, typeof(T));
