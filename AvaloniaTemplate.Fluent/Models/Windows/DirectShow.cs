...
namespace AvaloniaTemplate.Fluent.Models.Windows;
...
#pragma warning restore CS8625
	...
	public interface ICaptureGraphBuilder2
	{
		...
		int FindInterface([In] ref Guid pCategory, [In] ref Guid pType, [In] IBaseFilter pbf, [In] nint riid,
		...
		int ControlStream([In] ref Guid pCategory, [In] ref Guid pType, [In] IBaseFilter pFilter,
			[In] nint pstart, [In] nint pstop, [In] short wStartCookie, [In] short wStopCookie);
		...
		int FindPin([In] object pSource, [In] int pindir, [In] ref Guid pCategory, [In] ref Guid pType,
			[In, MarshalAs(UnmanagedType.Bool)] bool fUnconnected, [In] int num, [Out] out nint ppPin);
	}
	...
	public interface IAMStreamConfig
	{
		...
		int GetStreamCaps(int iIndex, [In, Out, MarshalAs(UnmanagedType.LPStruct)]
				ref AM_MEDIA_TYPE? ppmt, nint pSCC);
	}
	...
	{
		int GetPointer(ref nint ppBuffer);
	...
	public interface IReferenceClock
	{
		...
		int AdviseTime(long baseTime, long streamTime, nint hEvent, ref int pdwAdviseCookie);

		int AdvisePeriodic(long startTime, long periodTime, nint hSemaphore, ref int pdwAdviseCookie);
	...
	public interface ISampleGrabber
	{
		...
		int GetCurrentBuffer(ref int pBufferSize, nint pBuffer);

		int GetCurrentSample(nint ppSample);
	...
	public interface ISampleGrabberCB
	{
		...
		[PreserveSig()]
		int BufferCB(double sampleTime, nint pBuffer, int bufferLen);
	}
