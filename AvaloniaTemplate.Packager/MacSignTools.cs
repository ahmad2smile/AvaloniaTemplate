...
public static class MacSignTools
{
	public static void Sign(ArgsProcessor argsProcessor)
	{
		...
		var srcZipFileNamePattern = "AvaloniaToNotarize-*";
		...
		foreach (var zipPath in files)
		{
			...
			var versionPrefix = Path.GetFileNameWithoutExtension(zipPath).Split('-')[1]; // Example: "AvaloniaToNotarize-2.0.0.0-arm64.zip or AvaloniaToNotarize-2.0.0.0.zip ".
			var workingDir = Path.Combine(desktopPath, "avaloniaTemp");
			...
			var dmgFileName = zipFile.Replace("AvaloniaToNotarize", "Avalonia").Replace("zip", "dmg");
			var dmgFilePath = Path.Combine(workingDir, dmgFileName);
			var dmgUnzippedFilePath = Path.Combine(workingDir, $"Avalonia.tmp.dmg");
			var appNotarizeFilePath = Path.Combine(workingDir, $"Avalonia-{versionPrefix}.zip");
			...
			File.Copy(Path.Combine(contentsPath, "AvaloniaLogo.icns"), Path.Combine(dmgPath, ".VolumeIcon.icns"), true);
