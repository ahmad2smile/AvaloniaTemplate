...
/// </list>
/// <seealso href="https://github.com/zkSNACKs/AvaloniaTemplate/blob/master/AvaloniaTemplate.Documentation/ClientDeployment.md"/>
/// </summary>
public static class Program
{
	...
	private const string AvaloniaPrivateKeyFilePath = @"C:\avalonia\Avalonia.privkey";
	private const string AvaloniaPublicKeyFilePath = @"C:\avalonia\Avalonia.pubkey";
	...
	public static string DesktopProjectDirectory { get; } = Path.GetFullPath(Path.Combine(SolutionDirectory, "AvaloniaTemplate.Fluent.Desktop"));
	public static string LibraryProjectDirectory { get; } = Path.GetFullPath(Path.Combine(SolutionDirectory, "AvaloniaTemplate"));
	public static string WixProjectDirectory { get; } = Path.GetFullPath(Path.Combine(SolutionDirectory, "AvaloniaTemplate.WindowsInstaller"));
	...
	private static async Task Main(string[] args)
	{
		...
		{
			await AvaloniaSignerHelpers.GeneratePrivateAndPublicKeyToFileAsync(AvaloniaPrivateKeyFilePath, AvaloniaPublicKeyFilePath).ConfigureAwait(false);
			return;
		}
	...
	private static async Task SignAsync()
	{
		foreach (string target in Targets)
		{
			if (target.StartsWith("win", StringComparison.OrdinalIgnoreCase))
			{
				...
				var msiPath = Path.Combine(WixProjectDirectory, "bin", "Release", "Avalonia.msi");
				...
				{
					throw new Exception(".msi does not exist. Expected path: Avalonia.msi.");
				}
				...
				StartProcessAndWaitForExit("cmd", BinDistDirectory, $"signtool sign /d \"Avalonia Template\" /f \"{PfxPath}\" /p {{pfxPassword}} /t http://timestamp.digicert.com /a \"{newMsiPath}\" && exit");
			...
			{
				string dmgFileName = target.Contains("arm") ? $"Avalonia-{VersionPrefix}.dmg" : $"Avalonia-{VersionPrefix}-arm64.dmg";
		...
		using var key = await AvaloniaSignerHelpers.GetPrivateKeyFromFileAsync(AvaloniaPrivateKeyFilePath).ConfigureAwait(false);
		...
		await AvaloniaSignerHelpers.SignSha256SumsFileAsync(sha256sumAscFilePath, key).ConfigureAwait(false);

		// Verify back the signature file.
		await AvaloniaSignerHelpers.VerifySha256SumsFileAsync(sha256sumAscFilePath).ConfigureAwait(false);

		// Verify back Avalonia installer's hashes
		await AvaloniaSignerHelpers.VerifyInstallerFileHashesAsync(finalFiles, sha256sumAscFilePath).ConfigureAwait(false);
	...
	private static async Task PublishAsync()
	{
		...
		foreach (string target in Targets)
		{
			...
			if (target.StartsWith("win"))
			{
				oldExecutablePath = Path.Combine(currentBinDistDirectory, "AvaloniaTemplate.Fluent.Desktop.exe");
				newExecutablePath = Path.Combine(currentBinDistDirectory, $"{ExecutableName}.exe");

				// Delete unused executables.
				File.Delete(Path.Combine(currentBinDistDirectory, "AvaloniaTemplate.Fluent.exe"));
			}
			else // Linux & OSX
			{
				oldExecutablePath = Path.Combine(currentBinDistDirectory, "AvaloniaTemplate.Fluent.Desktop");
				newExecutablePath = Path.Combine(currentBinDistDirectory, ExecutableName);

				// Delete unused executables.
				File.Delete(Path.Combine(currentBinDistDirectory, "AvaloniaTemplate.Fluent"));
			}
			...
			if (target.StartsWith("win"))
			{
				ZipFile.CreateFromDirectory(currentBinDistDirectory, Path.Combine(deliveryPath, $"Avalonia-{deterministicFileNameTag}-{GetPackageTargetPostfix(target)}.zip"));
			...
			else if (target.StartsWith("osx"))
			{
				...
				var zipFileName = $"AvaloniaToNotarize-{deterministicFileNameTag}{postfix}.zip";
			...
			else if (target.StartsWith("linux"))
			{
				...
				var newFolderName = $"Avalonia-{VersionPrefix}";
				...
				newFolderName = "avaloniawallet";
				var linuxAvaloniaWalletFolder = Tools.LinuxPathCombine(linuxUsrLocalBinFolder, newFolderName);
				...
				{
					var number = file.Name.Split(new string[] { "AvaloniaLogo", ".png" }, StringSplitOptions.RemoveEmptyEntries);
				...
				var controlFileContent = $"Package: {ExecutableName}\n" +
					...
					$"Homepage: https://avaloniawallet.io\n" +
					$"Vcs-Git: git://github.com/zkSNACKs/AvaloniaTemplate.git\n" +
					$"Vcs-Browser: https://github.com/zkSNACKs/AvaloniaTemplate\n" +
					$"Architecture: amd64\n" +
				...
				var desktopFileContent = $"[Desktop Entry]\n" +
					...
					$"StartupWMClass=Avalonia Template\n" +
					...
					$"Keywords=bitcoin;wallet;crypto;blockchain;avalonia;privacy;anon;awesome;\n";

				File.WriteAllText(desktopFilePath, desktopFileContent, Encoding.ASCII);

				var avaloniaStarterScriptPath = Path.Combine(debUsrLocalBinFolderPath, $"{ExecutableName}");
				var avaloniaStarterScriptContent = $"#!/bin/sh\n" +
					$"{linuxAvaloniaWalletFolder.TrimEnd('/')}/{ExecutableName} $@\n";

				File.WriteAllText(avaloniaStarterScriptPath, avaloniaStarterScriptContent, Encoding.ASCII);
				...
				string newDeb = Path.Combine(BinDistDirectory, $"Avalonia-{VersionPrefix}.deb");
