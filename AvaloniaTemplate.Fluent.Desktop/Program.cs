...
using AvaloniaTemplate.Fluent.CrashReport;
...
using LogLevel = AvaloniaTemplate.Logging.LogLevel;
using System.Diagnostics.CodeAnalysis;
using AvaloniaTemplate.Fluent.Desktop.Extensions;
using System.Net.Sockets;
using System.Collections.ObjectModel;

namespace AvaloniaTemplate.Fluent.Desktop;
...
public class Program
{
	...
	public static int Main(string[] args)
	{
		...
		string dataDir = EnvironmentHelpers.GetDataDir(Path.Combine("AvaloniaTemplate", "Client"));
		SetupLogger(dataDir, args);

		Logger.LogDebug($"Avalonia was started with these argument(s): {(args.Any() ? string.Join(" ", args) : "none")}.");
		...
		try
		{
			...
			Logger.LogSoftwareStarted("Avalonia GUI");
			AppBuilder
				.Configure(() => new App())
				.UseReactiveUI()
				.SetupAppBuilder()
		...
		Logger.LogSoftwareStopped("Avalonia");

		return exceptionToReport is { } ? 1 : 0;
	}

	/// <summary>
	/// Initializes Avalonia Logger. Sets user-defined log-level, if provided.
	/// </summary>
	/// <example>Start Avalonia Template with <c>./wassabee --LogLevel=trace</c> to set <see cref="LogLevel.Trace"/>.</example>
	...
	{
		Logger.LogSoftwareStopped("Avalonia GUI");
	...
	private static AppBuilder BuildCrashReporterApp(SerializableException serializableException)
	{
		...
		return result
			...
			.With(new X11PlatformOptions { UseGpu = false, WmClass = "Avalonia Template Crash Reporting" })
