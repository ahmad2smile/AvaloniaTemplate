...
namespace AvaloniaTemplate.Services.Terminate;
...
public class TerminateService
{
	...
	public TerminateService(Func<Task> terminateApplicationAsync, Action terminateApplication)
	{
		...
		{
			// If the debugger is attached and you subscribe to SystemEvents, then on quit Avalonia gracefully stops but never returns from console.
	...
	private void Windows_SystemEvents_SessionEnding(object sender, SessionEndingEventArgs e)
	{
		...
		{
			// This event will only be triggered if you run Avalonia from the published package. Use the packager with the --onlybinaries option.
			Logger.LogInfo($"Process termination was requested by the OS, reason '{e.Reason}'.");
			e.Cancel = true;
		}

		// This must be a blocking call because after this the OS will terminate the Avalonia process if it exists.
	...
	private void CurrentDomain_ProcessExit(object? sender, EventArgs e)
	{
		...
		// This must be a blocking call because after this the OS will terminate Avalonia process if exists.
		Terminate();
	}
	...
	private void Console_CancelKeyPress(object? sender, ConsoleCancelEventArgs e)
	{
		...
		// This must be a blocking call because after this the OS will terminate Avalonia process if it exists.
