...
{
	private const string AvaloniaMagicString = "InBitcoinWeTrust";
	...
	/// <summary>
	/// Creates an object to ensure mutual exclusion of Avalonia instances per Network <paramref name="network"/>.
	/// The solution based on TCP socket.
	/// </summary>
	/// <param name="network">Bitcoin network selected when Avalonia Template was started. It will use the port 37129, 37130, 37131 according to network main, test, reg.</param>
	...
	/// On macOS this function will never throw if you run Avalonia as a macApp, because mac prevents running the same APP multiple times on OS level.
	/// </summary>
	/// <exception cref="InvalidOperationException">Avalonia is already running, signaling the first instance failed.</exception>
	/// <exception cref="OperationCanceledException">Avalonia is already running and signaled.</exception>
	public async Task EnsureSingleOrThrowAsync()
	{
		...
		catch (SocketException ex) when (ex.ErrorCode is 10048 or 48 or 98)
		{
			...
			// It is already used -> another Avalonia is running on this network.
			Logger.LogDebug("Another Avalonia instance is already running.");
		}
...
#pragma warning restore CA2007 // Consider calling ConfigureAwait on the awaited task
			...
			await writer.WriteAsync(AvaloniaMagicString.AsMemory(), cts.Token).ConfigureAwait(false);
		...
		catch (Exception ex)
		{
			...
			throw new InvalidOperationException($"Avalonia is already running, but cannot be signaled, reason: '{ex}'");
		}

		throw new OperationCanceledException($"Avalonia is already running, signaled the first instance.");
	}
...
#pragma warning restore CA2007 // Consider calling ConfigureAwait on the awaited task
					...
					if (answer == AvaloniaMagicString)
					{
						Logger.LogInfo($"Detected another Avalonia instance.");
						OtherInstanceStarted?.Invoke(this, EventArgs.Empty);
					}
				}
				catch (Exception ex)
				{
					// Somebody connected but it was not another Avalonia instance.
					Logger.LogDebug(ex);
				}
			}
		}
